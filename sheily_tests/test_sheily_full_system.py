...
def test_full_user_flow(client, test_user):
    ...
    login_response = client.post("/auth/login", 
                               json={"username": "fullsystemuser", "password": "fullsystempass"})
    ...
    add_tokens_response = client.post("/tokens/add", 
                                    json={"amount": 15}, 
                                    headers={"Authorization": f"Bearer {token}"})
    ...
    restore_response = client.post("/auth/restore", 
                                 json={"backup_id": backup_id, "password": "backup_password"})
...
def test_offline_operation(client, test_user):
    ...
    sync_response = client.post("/tokens/sync", 
                              headers={"Authorization": f"Bearer {token}"})
