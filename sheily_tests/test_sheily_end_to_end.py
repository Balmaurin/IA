...
def test_chat_conversation_flow(client, test_user):
    ...
    # 1. Login
    login_response = client.post("/auth/login", json={"username": "endtoenduser", "password": "endtoendpass"})
    token = login_response.json()["access_token"]

    # 2. Iniciar conversación
    messages = ["Hello, how are you?", "What can you do?", "Tell me about yourself"]

    responses = []
    for message in messages:
        response = client.post("/chat", json={"message": message}, headers={"Authorization": f"Bearer {token}"})
...
def test_token_earning_flow(client, test_user):
    ...
    balance_response = client.get("/tokens/balance", headers={"Authorization": f"Bearer {token}"})
    initial_balance = balance_response.json()["balance"]

    # 3. Realizar actividad que gana tokens (ej. completar tarea)
    task_response = client.post(
        "/tasks/complete", json={"task_type": "test_task"}, headers={"Authorization": f"Bearer {token}"}
    )
    ...
    spend_response = client.post(
        "/tokens/spend", json={"amount": 5, "reason": "premium_feature"}, headers={"Authorization": f"Bearer {token}"}
    )
...
def test_backup_restore_flow_with_chat_history(client, test_user):
    ...
    for msg in messages:
        client.post("/chat", json={"message": msg}, headers={"Authorization": f"Bearer {token}"})

    # 3. Crear backup
    backup_response = client.post(
        "/auth/backup", json={"password": "backup_pass"}, headers={"Authorization": f"Bearer {token}"}
    )
    ...
    # 5. Restaurar backup
    restore_response = client.post("/auth/restore", json={"backup_id": backup_id, "password": "backup_pass"})
