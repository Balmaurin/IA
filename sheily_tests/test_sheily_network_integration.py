...
def test_user_registration_sync(client, test_user):
    ...
    # 1. Login local
    login_response = client.post("/auth/login", json={"username": "networkuser", "password": "networkpass"})
...
def test_token_synchronization(client, test_user):
    ...
    add_response = client.post("/tokens/add", json={"amount": 10}, headers={"Authorization": f"Bearer {token}"})
...
def test_chat_fallback_to_central(client, test_user):
    ...
    chat_response = client.post(
        "/chat", json={"message": "Test with local AI down"}, headers={"Authorization": f"Bearer {token}"}
    )
...
def test_offline_operation_with_queuing(client, test_user):
    ...
    add_response = client.post("/tokens/add", json={"amount": 15}, headers={"Authorization": f"Bearer {token}"})
    assert add_response.status_code == 200

    # 4. Simular reconexión

    # 5. Forzar sincronización
    sync_response = client.post("/comms/sync", headers={"Authorization": f"Bearer {token}"})
