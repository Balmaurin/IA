...
ConsoleRenderableClass = TypeVar("ConsoleRenderableClass", bound=Type[ConsoleRenderable])
...
class BaseStyle:
    ...
    def _get_animation_colors(
        ...
        **metadata: Any,
        ...
        if animated and base_color.triplet is not None:
            colors = [lighten(base_color, self.brightness_multiplier * i) for i in range(0, steps)]
    ...
    def get_cursor_offset_for_element(self, element: Element, parent: Optional[Element] = None) -> CursorOffset:
    ...
    def render_input(
        ...
        text = self.render_input_value(element, is_active=is_active, parent=parent, done=done)
    ...
    def render_menu(
        ...
        if done:
            ...
            result_content.append(self.render_input_label(element, is_active=is_active, parent=parent))
    ...
    def render_progress(
        ...
        if element.logs and element._inline_logs:
            lines_to_show = element.logs[-element.lines_to_show :] if element.lines_to_show > 0 else element.logs
    ...
    def render_progress_log_line(
        ...
        brightness_multiplier = ((index + 1) / shown_lines) * (1.0 - current_min_brightness) + current_min_brightness
