...
class Container(Element):
    ...
    def _refresh(self, done: bool = False):
        ...
        should_show_cursor = (
            active_element.should_show_cursor if hasattr(active_element, "should_show_cursor") else False
        )
    ...
    def _get_element_position(self, element_index: int) -> int:
        ...
        for i in range(element_index + 1):
            ...
            if i == element_index:
                position += self.style.get_cursor_offset_for_element(current_element, parent=self).top
            else:
    ...
    def move_cursor_to_active_element(self) -> Tuple[Control, ...]:
        ...
        move_cursor = (Control((ControlType.CURSOR_UP, move_up)),) if move_up > 0 else ()

        cursor_left = self.style.get_cursor_offset_for_element(self._active_element, parent=self).left
