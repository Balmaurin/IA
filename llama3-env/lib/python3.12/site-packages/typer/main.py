...
def get_command(typer_instance: Typer) -> click.Command:
    ...
    if (
        ...
        if typer_instance._add_completion:
            ...
            click_command.params.append(click_show_param)
        return click_command
    raise RuntimeError("Could not get a command for this Typer instance")  # pragma: no cover
    ...
    if isinstance(parameter_info, OptionInfo):
        ...
        return (
            TyperOption(
                ...
                callback=get_param_callback(callback=parameter_info.callback, convertor=convertor),
...
def get_param_completion(
    ...
    if unassigned_params:
        ...
        raise click.ClickException(f"Invalid autocompletion callback parameters: {show_params}")
...
def launch(url: str, wait: bool = False, locate: bool = False) -> int:
    ...
    if url.startswith("http://") or url.startswith("https://"):
        if _is_macos():
            return subprocess.Popen(["open", url], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT).wait()

        has_xdg_open = _is_linux_or_bsd() and shutil.which("xdg-open") is not None

        if has_xdg_open:
            return subprocess.Popen(["xdg-open", url], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT).wait()
