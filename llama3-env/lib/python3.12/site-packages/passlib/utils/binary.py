...
class Base64Engine(object):
    ...
    def decode_int24(self, source):
        ...
        try:
            if self.big:
                return (
                    decode(source[3]) + (decode(source[2]) << 6) + (decode(source[1]) << 12) + (decode(source[0]) << 18)
                )
            else:
                return (
                    decode(source[0]) + (decode(source[1]) << 6) + (decode(source[2]) << 12) + (decode(source[3]) << 18)
                )
        except KeyError:
            raise ValueError("invalid character")
    ...
    def _encode_int(self, value, bits):
        ...
        return join_byte_elems(imap(self._encode64, ((value >> off) & 0x3F for off in itr)))
    ...
    def encode_int24(self, value):
        ...
        if value < 0 or value > 0xFFFFFF:
            raise ValueError("value out of range")
        raw = [value & 0x3F, (value >> 6) & 0x3F, (value >> 12) & 0x3F, (value >> 18) & 0x3F]
        if self.big:
            raw = reversed(raw)
        return join_byte_elems(imap(self._encode64, raw))
