...
def saslprep(source, param="value"):
    ...
    data = join_unicode(_USPACE if in_table_c12(c) else c for c in source if not in_table_b1(c))
    ...
    for c in data:
        ...
        if in_table_c8(c):
            raise ValueError("display-modifying / deprecated chars " "forbidden in" + param)
        if in_table_c9(c):
...
if stringprep is None:  # pragma: no cover -- runtime detection
    ...
    def saslprep(source, param="value"):
        ...
        raise NotImplementedError(
            "saslprep() support requires the 'stringprep' " "module, which is " + _stringprep_missing_reason
        )
...
def test_crypt(secret, hash):
    ...
    assert isinstance(hash, unicode_or_str), "hash must be unicode_or_str, got %s" % type(hash)
