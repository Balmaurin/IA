...
class _CommonFile(object):
    ...
    def __init__(
        ...
        if not encoding:
            warn(
                "``encoding=None`` is deprecated as of Passlib 1.6, "
        ...
        if not autoload:
            warn(
                "``autoload=False`` is deprecated as of Passlib 1.6, "
    ...
    def load(self, path=None, force=True):
        ...
        if path is not None:
            ...
            warn(
                "%(name)s.load(force=False) is deprecated as of Passlib 1.6,"
                "and will be removed in Passlib 1.8; "
                "use %(name)s.load_if_changed() instead." % dict(name=self.__class__.__name__),
                DeprecationWarning,
                stacklevel=2,
            )
...
def _init_default_schemes():
    ...
    if not bcrypt:
        _warn_no_bcrypt.update(["portable_apache_24", "host_apache_24", "linux_apache_24", "portable", "host"])
...
class HtpasswdFile(_CommonFile):
    ...
    def __init__(self, path=None, default_scheme=None, context=htpasswd_context, **kwds):
        if "default" in kwds:
            warn(
                "``default`` is deprecated as of Passlib 1.6, "
