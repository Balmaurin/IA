...
try:
    ...
    if not hasattr(_argon2_cffi, "Type"):
        ...
        _argon2_cffi_error = (
            "'argon2' module points to unsupported 'argon2' pypi package; " "please install 'argon2-cffi' instead."
        )
        _argon2_cffi = None
...
class _Argon2Common(
        ...
        **kwds,
        ...
        if digest_size is not None:
            ...
            subcls.checksum_size = uh.norm_integer(
                subcls, digest_size, min=16, max=MAX_UINT32, param="digest_size", relaxed=relaxed
            )
    ...
    def _validate_constraints(cls, memory_cost, parallelism):
        ...
        if memory_cost < min_memory_cost:
            raise ValueError(
                ...
                "8 * parallelism (8 * %d = %d)" % (cls.name, memory_cost, parallelism, min_memory_cost)
            )
    ...
    def _adapt_backend_error(cls, err, hash=None, self=None):
        ...
        text = str(err)
        if text not in ["Decoding failed"]:  # argon2_cffi's default message
...
class _PureBackend(_Argon2Common):
    ...
    def _load_backend_mixin(mixin_cls, name, dryrun):
        ...
        try:
            ...
            log.warning(
                "detected 'argon2pure' backend, but package is too old " "(passlib requires argon2pure >= 1.2.3)"
            )
            return False
