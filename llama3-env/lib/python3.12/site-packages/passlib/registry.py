...
_forbidden_names = frozenset(["onload", "policy", "context", "all", "default", "none", "auto"])
...
def get_crypt_handler(name, default=_UNSET):
    ...
    if alt != name:
        warn(
            "handler names should be lower-case, and use underscores instead " "of hyphens: %r => %r" % (name, alt),
            PasslibWarning,
            stacklevel=2,
        )
        name = alt
...
def get_supported_os_crypt_schemes():
    ...
    if not cache:  # pragma: no cover -- sanity check
        ...
        warn(
            ...
            "formats known to passlib! (system=%r release=%r)" % (platform.system(), platform.release()),
