...
"""
    ...
    def setUp(self):
        ...
        warnings.filterwarnings("ignore", r"the method.*hash_needs_update.*is deprecated")
...
class CryptContextTest(TestCase):
    ...
    def setUp(self):
        TestCase.setUp(self)
        warnings.filterwarnings("ignore", r"CryptContext\(\)\.replace\(\) has been deprecated.*")
        warnings.filterwarnings("ignore", r"The CryptContext ``policy`` keyword has been deprecated.*")
    ...
    def test_12_hash_needs_update(self):
        ...
        # check min rounds
        self.assertTrue(
            cc.hash_needs_update("$5$rounds=1999$jD81UCoo.zI.UETs$Y7qSTQ6mTiU9qZB4fRr43wRgQq4V.5AAf7F97Pzxey/")
        )
        self.assertFalse(
            cc.hash_needs_update("$5$rounds=2000$228SSRje04cnNCaQ$YGV4RYu.5sNiBvorQDlO0WWQjyJVGKBcJXz3OtyQ2u8")
        )

        # check max rounds
        self.assertFalse(
            cc.hash_needs_update("$5$rounds=3000$fS9iazEwTKi7QPW4$VasgBC8FqlOvD7x2HhABaMXCTh9jwHclPA9j5YQdns.")
        )
        self.assertTrue(
            cc.hash_needs_update("$5$rounds=3001$QlFHHifXvpFX4PLs$/0ekt7lSs/lOikSerQ0M/1porEHxYq7W/2hdFpxA3fA")
        )
