...
sample_hashes = dict(
    django_pbkdf2_sha256=(
        "not a password",
        django_pbkdf2_sha256.using(rounds=stock_config.get("django_pbkdf2_sha256__default_rounds")).hash(
            "not a password"
        ),
    )
)
        ...
        for obj, attr, source, patched in self._iter_patch_candidates():
            if patched:
                ...
                self.assertFalse(
                    ...
                    "obj=%r attr=%r should not have been patched: %r" % (obj, attr, source),
                )
