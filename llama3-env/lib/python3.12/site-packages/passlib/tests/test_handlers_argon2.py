...
class _base_argon2_test(HandlerCase):
    ...
    def test_keyid_parameter(self):
        ...
        self.assertRaises(
            ...
            self.handler.verify,
            "password",
            "$argon2i$v=19$m=65536,t=2,p=4,keyid=ABCD$c29tZXNhbHQ$" "IMit9qkFULCMA/ViizL57cnTLOa5DiVM9eMwpAvPwr4",
        )
    ...
    def test_type_using(self):
        ...
        for alt_type in handler.type_values:
            ...
            raise AssertionError(
                "expected to find alternate type: default=%r values=%r" % (orig_type, handler.type_values)
            )
    ...
    def test_argon_byte_encoding(self):
        ...
        # 8 byte salt
        salt = b"somesalt"
        temp = handler.using(memory_cost=256, time_cost=2, parallelism=2, salt=salt, checksum_size=32, type="i")
        hash = temp.hash("password")
        self.assertEqual(
            hash, "$argon2i$v=19$m=256,t=2,p=2" "$c29tZXNhbHQ" "$T/XOJ2mh1/TIpJHfCdQan76Q5esCFVoT5MAeIM1Oq2E"
        )
        ...
        hash = temp.hash("password")
        self.assertEqual(
            hash, "$argon2i$v=19$m=256,t=2,p=2" "$c29tZXNhbHQAAAAAAAAAAA" "$rqnbEp1/jFDUEKZZmw+z14amDsFqMDC53dIe57ZHD38"
        )
...
class argon2_argon2_cffi_test(_base_argon2_test.create_backend_case("argon2_cffi")):
    ...
    known_correct_hashes = _base_argon2_test.known_correct_hashes + [
        ...
        # v1.3, type ID
        ("password", "$argon2id$v=19$m=65536,t=2,p=4$c29tZXNhbHQ$" "GpZ3sK/oH9p7VIiV56G/64Zo/8GaUw434IimaPqxwCo"),
        #
        # custom
        #
        # ensure trailing null bytes handled correctly
    ...
    known_correct_hashes.extend(
        (info["secret"], info["hash"]) for info in reference_data if info["logM"] <= (18 if TEST_MODE("full") else 16)
    )
