        ...
        with self._random_global_lock:
            ...
            if global_seed is None:
                ...
                global_seed = TestCase._random_global_seed = int(
                    os.environ.get("RANDOM_TEST_SEED") or os.environ.get("PYTHONHASHSEED") or sys_rng.getrandbits(32)
                )
...
class HandlerCase(TestCase):
    ...
    def check_verify(self, secret, hash, msg=None, negate=False):
        ...
        if self.handler.is_disabled or negate:
            if not result:
                return
            if not msg:
                msg = "verify incorrectly returned True: secret=%r, hash=%r" % (secret, hash)
        ...
        for secret in self.stock_passwords:
            ...
            if self.handler.is_disabled and self.disabled_contains_salt:
                self.assertNotEqual(
                    other,
                    result,
                    "genhash() failed to salt result " "hash: secret=%r hash=%r: result=%r" % (secret, result, other),
                )
            else:
                self.assertEqual(
                    other,
                    result,
                    "genhash() failed to reproduce " "hash: secret=%r hash=%r: result=%r" % (secret, result, other),
                )
            ...
            if self.handler.is_disabled and not self.disabled_contains_salt:
                ...
                self.assertNotEqual(
                    ...
                    result,
                    "genhash() duplicated "
                    "hash: secret=%r hash=%r wrong_secret=%r: result=%r" % (secret, result, wrong_secret, other),
                )
    ...
    def test_10_optional_salt_attributes(self):
        ...
        if "salt_size" not in cls.setting_kwds and (not mx_set or cls.default_salt_size < cls.max_salt_size):
            warn(
                "%s: hash handler supports range of salt sizes, " "but doesn't offer 'salt_size' setting" % (cls.name,)
            )
    ...
    def test_11_unique_salt(self):
        ...
        def sampler(func):
            ...
            for _ in irange(samples):
                ...
                if value1 != value2:
                    return
            raise self.failureException("failed to find different salt after " "%d samples" % (samples,))
    ...
    def test_14_salt_chars(self):
        ...
        for c in source:
            if c not in cs:
                self.assertRaises(
                    ValueError, self.do_stub_encrypt, salt=c * chunk, __msg__="invalid salt char %r:" % (c,)
                )
    ...
    def test_30_HasManyIdents(self):
        ...
        self.assertTrue(
            cls.default_ident in cls.ident_values, "cls.default_ident must specify member of cls.ident_values"
        )

        # check optional aliases list
        if cls.ident_aliases:
            for alias, ident in iteritems(cls.ident_aliases):
                self.assertIsInstance(alias, unicode, "cls.ident_aliases keys must be unicode:")  # XXX: allow ints?
                self.assertIsInstance(ident, unicode, "cls.ident_aliases values must be unicode:")
                self.assertTrue(
                    ident in cls.ident_values, "cls.ident_aliases must map to cls.ident_values members: %r" % (ident,)
                )
    ...
    def test_has_many_idents_using(self):
        ...
        for alt_ident in handler.ident_values:
            ...
            raise AssertionError(
                "expected to find alternate ident: default=%r values=%r" % (orig_ident, handler.ident_values)
            )
    ...
    def test_secret_w_truncate_size(self):
        ...
        for cand_hasher in [without_error, with_error]:
            ...
            self.assertFalse(
                self.do_verify(alt_short_secret, short_hash, handler=with_error), "truncate_size value is too large"
            )
    ...
    def test_70_hashes(self):
        ...
        self.assertTrue(
            ...
            "test must set at least one of 'known_correct_hashes' " "or 'known_correct_configs'",
        )
    ...
    def test_71_alternates(self):
        ...
        for alt, secret, hash in self.known_alternate_hashes:
            ...
            # secret should verify successfully against hash
            self.check_verify(
                secret, alt, "verify() of known alternate hash " "failed: secret=%r, hash=%r" % (secret, alt)
            )
    ...
    def test_72_configs(self):
        ...
        for config, secret, hash in self.known_correct_configs:
            ...
            self.assertTrue(
                self.do_identify(config), "identify() failed to identify known config string: %r" % (config,)
            )
    ...
    def test_73_unidentified(self):
        ...
        for hash in self.known_unidentified_hashes:
            ...
            self.assertFalse(
                self.do_identify(hash), "identify() incorrectly identified known unidentifiable " "hash: %r" % (hash,)
            )
            ...
            self.assertRaises(
                ...
                "stub",
                hash,
                __msg__="verify() failed to throw error for unidentifiable " "hash: %r" % (hash,),
            )
            ...
            self.assertRaises(
                ...
                "stub",
                hash,
                __msg__="genhash() failed to throw error for unidentifiable " "hash: %r" % (hash,),
            )
    ...
    def test_74_malformed(self):
        ...
        for hash in self.known_malformed_hashes:
            ...
            self.assertTrue(
                self.do_identify(hash), "identify() failed to identify known malformed " "hash: %r" % (hash,)
            )
            ...
            self.assertRaises(
                ...
                "stub",
                hash,
                __msg__="verify() failed to throw error for malformed " "hash: %r" % (hash,),
            )
            ...
            self.assertRaises(
                ...
                "stub",
                hash,
                __msg__="genhash() failed to throw error for malformed " "hash: %r" % (hash,),
            )
        ...
        for name, hash in self.known_other_hashes:
            ...
            if name == self.handler.name:
                ...
                self.assertFalse(
                    ...
                    "identify() incorrectly identified hash belonging to " "%s: %r" % (name, hash),
                )
                ...
                self.assertRaises(
                    ...
                    "stub",
                    hash,
                    __msg__="genhash() failed to throw error for hash " "belonging to %s: %r" % (name, hash),
                )
    ...
    def test_76_hash_border(self):
        ...
        for hash in [u(""), b""]:
            if self.accepts_all_hashes:
                ...
                self.assertRaises(
                    ValueError, self.do_verify, "stub", hash, __msg__="verify() failed to reject empty hash"
                )
                self.assertRaises(
                    ValueError, self.do_genhash, "stub", hash, __msg__="genhash() failed to reject empty hash"
                )
    ...
    def test_disable_and_enable(self):
        ...
        self.assertTrue(
            handler.identify(disabled_stub), msg="identify() didn't recognize disable() result: %r" % (disabled_stub)
        )
