...
from passlib.utils import join_bytes, to_native_str, join_byte_values, to_bytes, SequenceMixin, as_bool
...
class HashInfo(SequenceMixin):
    ...
    def __init__(self, const, names, required=True):  # *,
        ...
        if hash.name != self.name:
            warn(
                "inconsistent digest name: %r resolved to %r, which reports name as %r" % (self.name, const, hash.name),
...
def pbkdf2_hmac(digest, secret, salt, rounds, keylen=None):
    ...
    return join_bytes(
        calc_block(keyed_hmac, keyed_hmac(salt + _pack_uint32(i)), rounds) for i in irange(1, block_count + 1)
    )[:keylen]
