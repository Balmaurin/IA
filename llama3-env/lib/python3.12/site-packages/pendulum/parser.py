...
def _parse(text: str, **options: t.Any) -> Date | DateTime | Time | Duration | Interval[DateTime]:
    """
    ...
    if isinstance(parsed, _Interval):
        if parsed.duration is not None:
            ...
            dt = pendulum.instance(t.cast("datetime.datetime", parsed.end), tz=options.get("tz", UTC))
        ...
        return pendulum.interval(
            pendulum.instance(t.cast("datetime.datetime", parsed.start), tz=options.get("tz", UTC)),
            pendulum.instance(t.cast("datetime.datetime", parsed.end), tz=options.get("tz", UTC)),
        )
