...
def parse_iso8601(
    ...
    if m.group("date"):
        ...
        if m.group("isocalendar"):
            ...
            # by week number
            if m.group("weeksep") and not m.group("weekdaysep") and m.group("isoweekday"):
            ...
            try:
                date = _get_iso_8601_week(m.group("isoyear"), m.group("isoweek"), m.group("isoweekday"))
            except ParserError:
                raise
            except ValueError:
                raise ParserError(f"Invalid date string: {text}")
...
def _get_iso_8601_week(year: int | str, week: int | str, weekday: int | str) -> dict[str, int]:
