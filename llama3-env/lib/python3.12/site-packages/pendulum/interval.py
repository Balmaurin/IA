...
class Interval(Duration, Generic[_T]):
    ...
    def __new__(cls, start: _T, end: _T, absolute: bool = False) -> Self:
        ...
        if (
            ...
            and ((start.tzinfo is None and end.tzinfo is not None) or (start.tzinfo is not None and end.tzinfo is None))
        ):
        ...
        if isinstance(_start, datetime) and isinstance(_end, datetime) and _start.tzinfo is _end.tzinfo:
    ...
    def in_words(self, locale: str | None = None, separator: str = " ") -> str:
        ...
        for interval in intervals:
            ...
            if abs(interval_count) > 0:
                translation = loaded_locale.translation(f"units.{unit}.{loaded_locale.plural(abs(interval_count))}")
    ...
    def __reduce_ex__(self, protocol: SupportsIndex) -> tuple[type[Self], tuple[_T, _T, bool]]:
