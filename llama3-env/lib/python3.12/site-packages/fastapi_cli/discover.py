...
def get_default_path() -> Path:
    ...
    raise FastAPICLIException("Could not find a default file to run, please provide an explicit path")
...
def get_app_name(*, mod_data: ModuleData, app_name: Union[str, None] = None) -> str:
    ...
    if not FastAPI:  # type: ignore[truthy-function]
        raise FastAPICLIException("Could not import FastAPI, try running 'pip install fastapi'") from None
    object_names = dir(mod)
    object_names_set = set(object_names)
    if app_name:
        if app_name not in object_names_set:
            raise FastAPICLIException(f"Could not find app name {app_name} in {mod_data.module_import_str}")
...
def get_import_data(*, path: Union[Path, None] = None, app_name: Union[str, None] = None) -> ImportData:
    if not path:
