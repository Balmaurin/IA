    ...
    if not allow_truncate:
        if len(digest) > curve.baselen:
            raise BadDigestError(
                ...
                "for the length of your digest ({1})".format(curve.name, 8 * len(digest))
            )
    else:
...
class VerifyingKey(object):
    ...
    def __repr__(self):
        ...
        if self.default_hashfunc:
            ...
            hash_name = "None"
        return "VerifyingKey.from_string({0!r}, {1!r}, {2})".format(pub_key, self.curve, hash_name)
    ...
    def precompute(self, lazy=False):
        ...
        if isinstance(self.curve.curve, CurveEdTw):
            ...
            self.pubkey.point = ellipticcurve.PointJacobi.from_affine(self.pubkey.point, True)
    ...
    def from_der(
        ...
        if empty != b"":
            raise der.UnexpectedDER("trailing junk after DER pubkey: %s" % binascii.hexlify(empty))
        ...
        if not oid_pk == oid_ecPublicKey:
            raise der.UnexpectedDER("Unexpected object identifier in DER " "encoding: {0!r}".format(oid_pk))
        curve = Curve.from_der(rest, valid_curve_encodings)
        point_str, empty = der.remove_bitstring(point_str_bitstring, 0)
        if empty != b"":
            raise der.UnexpectedDER("trailing junk after pubkey pointstring: %s" % binascii.hexlify(empty))
    ...
    def from_public_key_recovery_with_digest(
        ...
        verifying_keys = [cls.from_public_point(pk.point, curve, hashfunc) for pk in pks]
...
class SigningKey(object):
    ...
    def from_secret_exponent(cls, secexp, curve=NIST192p, hashfunc=sha1):
        ...
        """
        if isinstance(curve.curve, CurveEdTw):
            raise ValueError("Edwards keys don't support setting the secret scalar " "(exponent) directly")
        self = cls(_error__please_use_generate=True)
        self.curve = curve
        self.default_hashfunc = hashfunc
        self.baselen = curve.baselen
        n = curve.order
        if not 1 <= secexp < n:
            raise MalformedPointError("Invalid value for secexp, expected integer " "between 1 and {0}".format(n))
        pubkey_point = curve.generator * secexp
        if hasattr(pubkey_point, "scale"):
            pubkey_point = pubkey_point.scale()
        self.verifying_key = VerifyingKey.from_public_point(pubkey_point, curve, hashfunc, False)
    ...
    def from_string(cls, string, curve=NIST192p, hashfunc=sha1):
        ...
        if len(string) != curve.baselen:
            raise MalformedPointError(
                "Invalid length of private key, received {0}, " "expected {1}".format(len(string), curve.baselen)
            )
        if isinstance(curve.curve, CurveEdTw):
            self = cls(_error__please_use_generate=True)
            self.curve = curve
            ...
            self.verifying_key = VerifyingKey.from_string(self.privkey.public_key().public_key(), curve)
            return self
    ...
    def from_der(cls, string, hashfunc=sha1, valid_curve_encodings=None):
        ...
        if empty != b"":
            raise der.UnexpectedDER("trailing junk after DER privkey: %s" % binascii.hexlify(empty))
        ...
        if der.is_sequence(s):
            if version not in (0, 1):
                raise der.UnexpectedDER("expected version '0' or '1' at start of privkey, got %d" % version)
            ...
            if algorithm_oid in (Ed25519.oid, Ed448.oid):
                ...
                if s:
                    raise der.UnexpectedDER("trailing junk after the encoded private key")
            ...
            if algorithm_oid not in (oid_ecPublicKey, oid_ecDH, oid_ecMQV):
                raise der.UnexpectedDER("unexpected algorithm identifier '%s'" % (algorithm_oid,))
        ...
        if version != 1:
            raise der.UnexpectedDER("expected version '1' at start of DER privkey, got %d" % version)
    ...
    def to_der(
        ...
        if format == "ssleay":
            priv_key_elems.append(der.encode_constructed(0, self.curve.to_der(curve_parameters_encoding)))
        # the 0 in encode_bitstring specifies the number of unused bits
        # in the `encoded_vk` string
        priv_key_elems.append(der.encode_constructed(1, der.encode_bitstring(encoded_vk, 0)))
